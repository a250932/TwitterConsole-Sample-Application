...
namespace TwitterConsole
{
    public struct TwitterConfig
    {
        ...
        public readonly string Resource_url;

        public TwitterConfig(string oauthToken, string oauthTokenSecret, string oauthConsumerKey, string oauthConsumerSecret, string keywords, string searchGroups, string resourceurl)
        {
    ...
    public class Tweet
    {
        ...
        [DataMember(Name = "entities")] public TweetEntity Entities;
        ...
        public IEnumerable<Tweet> StreamStatuses(TwitterConfig config)
        {
            ...
            while (keepRunning)
            {
                ...
                {
                    var result = JsonConvert.DeserializeObject<Tweet>(line);
                    result.RawJson = line;
                    yield return result;
                }
                if (line == null)
    ...
    public class Urls
    {
        ...
        {
            if (Expanded_Url.Contains(@"://"))
                Expanded_Url = Expanded_Url.Split(new string[] { "://" }, 2, StringSplitOptions.None)[1];

            return Expanded_Url.Split('/')[0];
        }

        private bool ContainsPic()
        {
            bool isPic = false;
            if (Expanded_Url.Contains(@"pic.twitter.com") || Expanded_Url.Contains("instagram"))
            {
                isPic = true;
